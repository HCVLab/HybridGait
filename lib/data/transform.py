...
class SmplSilTransform():
    def __init__(self, disvor=1.0, img_shape=None):
        self.disvor = disvor
...
class BaseSilDefaultTransform():
    def __init__(self, disvor=255.0):
        self.disvor = disvor
...
def get_transform(trf_cfg=None):
    if is_dict(trf_cfg):
        ...
        valid_trf_arg = get_valid_args(transform, trf_cfg, ['type', 'pytorch_d', 'pytorch_d_type'])
        transform = transform(**valid_trf_arg)
        # transform = get_transform(trf_cfg)
        return transform
    if trf_cfg is None:
        return lambda x: x
    if is_list(trf_cfg):
...
#         transform = getattr(base_transform, trf_cfg['type'])
#         valid_trf_arg = get_valid_args(transform, trf_cfg, ['type', 'pytorch_d', 'pytorch_d_type'])
#         if not trf_cfg['pytorch_d']:
#             return transform(**valid_trf_arg)
#         else:
#             if trf_cfg['pytorch_d_type'] == 'H_Flip':
#                 transform = transforms.Compose([
#                     BaseSilDefaultTransform(),
#                     transforms.RandomHorizontalFlip(),
#                 ])
#             elif trf_cfg['pytorch_d_type'] == 'V_Flip':
#                 transform = transforms.Compose([
#                     BaseSilDefaultTransform(),
#                     transforms.RandomVerticalFlip(),
#                 ])
#             elif trf_cfg['pytorch_d_type'] == 'Erasing':
#                 transform = transforms.Compose([
#                     BaseSilDefaultTransform(),
#                     transforms.RandomErasing(),
#                 ])
#             elif trf_cfg['pytorch_d_type'] == 'Rotate':
#                 transform = transforms.Compose([
#                     BaseSilDefaultTransform(),
