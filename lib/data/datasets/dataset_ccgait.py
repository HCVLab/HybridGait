...
class DataSet_CCGait(tordata.Dataset):
    ...
    def __loader__(self, paths):
        ...
        for pths in paths:
            sil_pth = pths[0]
            # raise ValueError('sil_pth:{}'.format(sil_pth))
            if sil_pth.endswith('.pkl'):
                try:
                    with open(sil_pth, 'rb') as f:
                        sil_data = pickle.load(f)
                        # raise ValueError('sil_data.shape:{}'.format(sil_data.shape))
                    f.close()
                except:
                    raise ValueError('sil_pth:{}'.format(sil_pth))
            sp_pth = pths[1]
            if sp_pth.endswith('.pkl'):
                ...
                f.close()
            sp_sil_pth = pths[2]
            if sp_sil_pth.endswith('.pkl'):
                with open(sp_sil_pth, 'rb') as f:
                    sp_sil_data = pickle.load(f)
                f.close()
            else:
                raise ValueError('- Loader - just support .pkl !!!')
            # data_list.append([sil_data])
            # data_list.append(sil_data)
            data_list.append([sil_data, sp_data, sp_sil_data])
        for idx, data in enumerate(data_list):
            ...
            if len(data[0]) != len(data[2]):
                raise ValueError(
                    'Each input sil data({}) should have the same length to smpl_sil data({}).'
                        .format(paths[idx][0], paths[idx][2]))
            if len(data[0]) == 0 or len(data[1]) == 0 or len(data[2]) == 0:
                raise ValueError(
                    'Each input sil data({}) and smpl data({}) and smpl_sil data({}) should have at least one element.'
                        .format(paths[idx][0], paths[idx][1], paths[idx][2]))
    ...
    def __load_seqs_into_list(self, dataset_root, smpl_root, smpl_sil_root, lab, cam, seq, seqs_info_list, data_in_use):
    # def __load_seqs_into_list(self, dataset_root, lab, seq, seqs_info_list, data_in_use):
        msg_mgr = get_msg_mgr()
        seq_paras = [lab, cam, seq]
        seq_path = osp.join(dataset_root, *seq_paras)
        smpl_path = osp.join(smpl_root, *seq_paras)
        smpl_sil_path = osp.join(smpl_sil_root, *seq_paras)

        seq_dirs = sorted(os.listdir(seq_path))

        seq_info = [lab, cam, seq]

        if seq_dirs != []:
            seq_dirs = [[osp.join(seq_path, dir), osp.join(smpl_path, dir), osp.join(smpl_sil_path, dir)]
                        for dir in seq_dirs]
            ...
            msg_mgr.log_debug('Find no .pkl file in %s-%s.' % (lab, seq))
    ...
    def __dataset_parser(self, data_config, training):
        ...
        smpl_sil_root = data_config['dataset_root']['smpl_sil_root']
        try:
        ...
        # print("probe_set:{}".format(probe_set))
                ...
                for cam in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for seq in sorted(os.listdir(osp.join(dataset_root, lab, cam))):
                        ...
                        if id_seq in probe_set:
                            ...
                            self.__load_seqs_into_list(dataset_root, smpl_root, smpl_sil_root,
                                                        lab, cam, seq,
                                                        probe_seqs_info_list, data_in_use)
                            continue
                        # self.__load_seqs_into_list(dataset_root,
