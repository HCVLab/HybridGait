...
from utils import evaluate_rank_LTGait
...
def evaluation_Gait3D(data, conf, probe_num, metric='euc'):
    ...
    # 
    probe_cams = np.asarray(cams[:probe_num])
    gallery_cams = np.asarray(cams[probe_num:])
    probe_time_seqs = np.asarray(time_seqs[:probe_num])
    gallery_time_seqs = np.asarray(time_seqs[probe_num:])

    results = OrderedDict()
    msg_mgr.log_info(f"The test metric you choose is {metric}.")
    # dist = cuda_dist2(probe_features, gallery_features, metric).cpu().numpy()
    ...
    cmc, all_AP, all_INP = evaluate_rank(dist, probe_lbls, gallery_lbls, probe_cams, gallery_cams, probe_time_seqs, gallery_time_seqs)
...
def evaluation_LTGait(data, conf, probe_num, metric='euc'):
    msg_mgr = get_msg_mgr()

    dataset_name = conf["data_cfg"]["test_dataset_name"]

    features, labels, cams = data['embeddings'], data['labels'], data['types']
    ...
    dist = cuda_dist(probe_features, gallery_features, metric).cpu().numpy()
    # cmc, all_AP, all_INP = evaluate_rank(dist, probe_lbls, gallery_lbls)
    cmc, all_AP, all_INP = evaluate_rank_LTGait(dist, probe_lbls, gallery_lbls, probe_cams, gallery_cams)
