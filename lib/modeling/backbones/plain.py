...
from ..modules import BasicConv2d, FocalConv2d, BasicConv3d
...
class Plain(nn.Module):
    ...
    def make_layers(self):
        ...
        def get_layer(cfg, in_c, kernel_size, stride, padding):
            cfg = cfg.split('-')
            typ = cfg[0]
            if typ not in ['BC', 'FC', 'BC3D']:
                raise ValueError('Only support BC, BC3D or FC, but got {}'.format(typ))
            out_c = int(cfg[1])

            if typ == 'BC':
                return BasicConv2d(in_c, out_c, kernel_size=kernel_size, stride=stride, padding=padding)
            elif typ == 'BC3D':
                return BasicConv3d(in_c, out_c, kernel_size=kernel_size, stride=stride, padding=padding)
            return FocalConv2d(in_c, out_c, kernel_size=kernel_size, stride=stride, padding=padding, halving=int(cfg[2]))
        if self.layers_cfg[0].split('-')[0] == 'BC':
            Layers = [get_layer(self.layers_cfg[0], self.in_channels,
                                5, 1, 2), nn.LeakyReLU(inplace=True)]
        elif self.layers_cfg[0].split('-')[0] == 'BC3D':
            Layers = [get_layer(self.layers_cfg[0], self.in_channels,
                                (5, 5, 3), 1, 2), nn.LeakyReLU(inplace=True)]
        ...
        for cfg in self.layers_cfg[1:]:
            if cfg == 'M':
                ...
                elif cfg.split('-')[0] == 'BC3D':
                    conv3d = get_layer(cfg, in_c, 3, 1, 1)
                    Layers += [conv3d, nn.LeakyReLU(inplace=True)]
                    in_c = int(cfg.split('-')[1])
